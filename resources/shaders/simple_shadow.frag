...
void main()
{
  ...
  // Vertex noise
  out_fragColor = vec4(surf.color, 1.0);

  // Fragment Noise
  // out_fragColor = vec4(vec3(noise(surf.wNorm * 100)), 1.0);
  ...
  //       noise(surf.wNorm + 20),
  //       noise(surf.wTangent),
  //       noise(cross(surf.wTangent, surf.wNorm)) 
  //   ), 1.0
  // );

  // Noise with lighting
  // out_fragColor = (lightColor*shadow + vec4(0.1f)) * out_fragColor;
}
